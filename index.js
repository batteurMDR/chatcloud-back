...
io.on('connection', (socket) => {
    ...
    socket.on('new message', (data) => {
        ...
        sequelize.query("INSERT INTO `messages`(`id`, `message`, `username`, `score`) VALUES (null, '" + data + "', '" + socket.username + "', 0)").then((message) => {
            ...
            const runPython = async (data, sequelize) => {
                ...
                await sequelize.query("UPDATE `messages` SET `score` = " + score + " WHERE `id` = '" + newMessageId + "'")
                 .then(() => {})
                 .catch(err => {
            ...
            try {
                (async () => {
                    ...
                    //  TRIGGER UN MAIL
                    await sequelize.query("UPDATE `users` SET `score` = " + generalScore + "WHERE `username`='" + socket.username + "'").then(() => {}).catch(err => {
    ...
    socket.on('add user', (username) => {
        ...
        sequelize.query("SELECT * FROM `users` WHERE `username`='" + socket.username + "'").then(([result]) => {
            if (result.length === 0) {
                sequelize.query("INSERT INTO `users` (`id`, `username`, `score`) VALUES (null, '" + username + "', 0)").then((user) => {
                    ...
                    sequelize.query('SELECT * FROM messages LIMIT 50').then(([result]) => {
                        socket.emit('previous messages', result.map((r) => ({username: r.username, message: r.message, score: r.score})));
                    });
                })
                .catch(err => {
                    if (err) console.log(err.message);
